...
DATASETS: list[DatasetInfo] = [
    ...
    DatasetInfo(
        ...
        desc_en="Index daily bars for a given trade_date (covers major exchange indices).",
        desc_zh="指数日线行情（指定交易日，全市场指数日线行情）。",
    ),
    DatasetInfo(
        name="etf_daily",
        category="market",
    ...
    DatasetInfo(
        name="dividend",
        category="finance",
        partitioning="end_date (yearly)",
        source="tushare: dividend",
        desc_en="Dividend distribution (分红送股) per ts_code and report period.",
        desc_zh="分红送股数据（按报告期）。",
    ),
    DatasetInfo(
        name="fina_indicator",
    ...
    DatasetInfo(
        ...
        desc_en="Financial audit opinion (财务审计意见) per ts_code and report period.",
        desc_zh="财务审计意见（按报告期）。",
    ),
    DatasetInfo(
        name="fina_mainbz",
