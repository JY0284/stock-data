...
def backfill(cfg: RunConfig, *, token: str, start_date: str, end_date: str, datasets: str) -> None:
    ...
    run_basic(cfg, token=token, catalog=cat, datasets=[d for d in selected if d in {"stock_basic", "trade_cal", "stock_company", "new_share", "namechange"}], start_date=start_date, end_date=end_date)

    # Market (uses trade_cal and is date-partitioned).
    run_market(cfg, token=token, catalog=cat, datasets=[d for d in selected if d not in {"stock_basic", "trade_cal", "stock_company", "new_share", "namechange"}], start_date=start_date, end_date=end_date)
...
def update(cfg: RunConfig, *, token: str, end_date: str, datasets: str) -> None:
    ...
    run_basic(cfg, token=token, catalog=cat, datasets=[d for d in selected if d in {"stock_basic", "trade_cal", "stock_company", "new_share", "namechange"}], start_date=None, end_date=end_date)

    # Market incremental (figures out missing days from ingestion_state + trade_cal).
    run_market(cfg, token=token, catalog=cat, datasets=[d for d in selected if d not in {"stock_basic", "trade_cal", "stock_company", "new_share", "namechange"}], start_date=None, end_date=end_date)
